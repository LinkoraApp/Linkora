...
@Dao
interface TagsDao {
    @Insert suspend fun createATag(tag: Tag): Long

    @Update suspend fun updateATag(tag: Tag)
    ...
    @Insert suspend fun createLinkTags(linksTags: List<LinkTag>)
    ...
    @Query(
        "SELECT tags.* FROM tags INNER JOIN link_tags ON tags.localId = link_tags.tagId WHERE link_tags.linkId = :linkId",
    )
