...
class LocalDatabaseUtilsImplTest {
    ...
    @Test
    fun `resetDatabase executes raw SQL to thoroughly truncate all active tables and sequences`() = runTest {
    ...
    @Test
    fun `getFoldersRowCount natively counts exactly the number of folders in the database`() = runTest {
    ...
    @Test
    fun `getChildFolderData correctly translates empty last seen id constant to null to avoid SQL constraint violations`() = runTest {
        val result =
    ...
    @Test
    fun `search with assignPath dynamically builds hierarchical ancestor tree and strictly reverses it for deep structures`() = runTest {
        val rootId =
            database.foldersDao.insertANewFolder(
                Folder(
                    name = "Root",
        ...
        val targetItem = searchData.find {
            it.folderParentId == childId ||
                (it.itemType == Constants.FOLDER && it.folderLocalId == grandChildId)
        }
    ...
    @Test
    fun `search without assignPath immediately emits raw results bypassing recursive folder path resolution`() = runTest {
        val rootId =
            database.foldersDao.insertANewFolder(
                Folder(
                    name = "Root",
        ...
        val searchData = (result as Result.Success).data
        val targetItem = searchData.find {
            it.folderParentId == rootId ||
                (it.itemType == Constants.FOLDER && it.folderLocalId == targetId)
        }
    ...
    @Test
    fun `search with assignPath specifically aborts path resolution early for non folder links`() = runTest {
        database.linksDao.addANewLink(
            Link(
                url = "https://xyz.com",
